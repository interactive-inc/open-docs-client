...
test("DocCustomSchemaFieldValue - テキストフィールドの作成と検証", () => {
  ...
  // 無効な値（requiredがtrueなので throw する）
  expect(field.validate(null)).toBe(null) // nullはparseされる
  expect(() => field.validate(undefined)).toThrow() // undefinedはエラーになり required なので throw
})
...
test("DocCustomSchemaFieldValue - ブール値フィールドの作成と検証", () => {
  ...
  // 無効な値（requiredがtrueなので throw する）
  expect(field.validate(null)).toBe(null) // nullはparseされる
  expect(() => field.validate("true")).toThrow() // 文字列はエラーになり required なので throw
  expect(() => field.validate("invalid")).toThrow()
})
...
test("DocCustomSchemaFieldValue - リレーションフィールドの作成と検証", () => {
  ...
  // 無効な値（requiredがtrueなので throw する）
  expect(field.validate(null)).toBe(null) // nullはparseされる
  expect(() => field.validate(123)).toThrow() // 数値はエラーになり required なので throw
})
...
test("DocCustomSchemaFieldValue - 複数値フィールドの作成と検証", () => {
  ...
  // 無効な値（requiredがtrueなので throw する）
  expect(() => multiTextField.validate("tag1,tag2")).toThrow() // 文字列はエラーになり required なので throw
  expect(() => multiTextField.validate(null)).toThrow() // nullはエラーになり required なので throw
  expect(() => multiTextField.validate(undefined)).toThrow() // undefinedはエラーになり required なので throw
